	...
	printf("\n\rTCP: Connect server successfully.");
        ...
        while (1) {
              ...
              if(rcvStatus > 0){
                    ...
                    memcpy(sendBuf,"i have receive",sizeof("i have receive"));
                    memcpy((char *)&sendBuf[sizeof("i have receive")-1],(char *)rcvBuf,rcvStatus);

                    printf("\n iStatus= %d ;", rcvStatus);
                    printf("\n rcvBuf = %s;",rcvBuf);
                    
                    lwip_send(iSockFD,sendBuf, sizeof("i have receive") + rcvStatus, 0);
...
int BsdTcpClientTest_2(const char *host_ip, unsigned short usPort)
{
    int                 iCounter;
    ...
    MsgFormat_t hostMsg;
    MsgFormat_t *pHostMsg = &hostMsg;
    
    
    /////////// test msg init ///////////
    
    hostMac = (u8 *)LwIP_GetMAC(&xnetif[0]);
    
    
    pHostMsg->msgStatus.msgNum = 0x00000000;
    pHostMsg->msgStatus.cmd = 10003;
    pHostMsg->msgStatus.len = 0;
    pHostMsg->data = sendBuf;
	...
	printf("\n\rTCP: Connect server successfully.");
        ...
        {      
              ...
              vTaskDelay(500);
              lwip_send(iSockFD,pHostMsg, sizeof(MsgStatus_t) + pHostMsg->msgStatus.len, 0);
...
void TcpClientHandler_test(void *param)
{
	...
	if(g_start_client)
		//BsdTcpClient_test("120.24.49.173", port);
                BsdTcpClientTest_2(/*"192.168.1.27"*/"120.24.49.173", port);
